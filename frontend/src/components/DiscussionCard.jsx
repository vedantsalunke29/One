...
export default function DiscussionCard({ item }) {
	...
	const handleLike = async (count) => {
		try {
			axios
				.post("https://one-jc5d.onrender.com/handle-like", {
					count,
					_id,
					regIdNo,
				})
	...
	const getLike = async () => {
		try {
			axios
				.post("https://one-jc5d.onrender.com/get-like", { _id })
				.then((res) => {
					if (res.data === "fail") {
					}
					setLikes(res.data);
	...
	const deleteDiscussion = async () => {
		try {
			axios.post("https://one-jc5d.onrender.com/delete-discussion", {
				_id,
				replyArray,
			});
		} catch (error) {
	...
	const submitReply = async () => {
		try {
			axios
				.post("https://one-jc5d.onrender.com/reply-to-discussion", {
					_id,
					regIdNo,
					replyMsg,
					replyArray,
	...
	const getReply = async () => {
		try {
			axios
				.post("https://one-jc5d.onrender.com/get-reply-to-discussion", {
					replyArray,
					_id,
				})
				.then((res) => {
